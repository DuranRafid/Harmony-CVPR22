...
def evaluate_model(dataset_name,model, z_dim, pixel, batch_size, device,scale):
    ...
    #save_output_images(dataset_name, train_loader, model, pixel, 'train', batch_size, device=device)
    #save_output_images(dataset_name, test_loader, model, pixel, 'test', batch_size, device=device)
    #save_latent_variables(dataset_name, train_loader, model, 'train', pixel,w)
...
def calculate_score(dataset_name,model, z_dim, pixel, scale, batch_size, device):
    ...
    return discrete_factor_prediction_score(train_semantic_z.reshape(-1,z_dim),train_labels,test_semantic_z.reshape(-1,z_dim),test_labels,'KNN')
